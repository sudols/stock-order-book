...
function hr(left = '╠', mid = '╬', right = '╣', fill = '═') {
  ...
  return left + fill.repeat(half) + mid + fill.repeat(LINE_WIDTH - 2 - half) + right;
}
...
function top()    { return '╔' + '═'.repeat(LINE_WIDTH - 2) + '╗'; }
function bottom() { return '╚' + '═'.repeat(LINE_WIDTH - 2) + '╝'; }
...
function render(snap: OrderBookSnapshot) {
  ...
  lines.push(top());
  lines.push(center(`LIVE ORDER BOOK — ${SYMBOL}`));
  lines.push(center(`Updated: ${fmt.time(snap.timestamp)}`));
  lines.push(hr('╠', '╦', '╣'));
...
socket.on('connect_error', (err: Error) => {
  ...
  console.error(`Make sure the server is running: pnpm --filter @orderbook/server dev`);
